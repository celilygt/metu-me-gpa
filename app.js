...
const navItems = [
  ...
  ["retakes", "repeat", "Tekrar almaya değer mi?"],
...
function render() {
  ...
  $("#app").innerHTML = `<aside class="sidebar">
    <a href="#overview" class="brand" data-page="overview"><span class="brand-icon">${icon("compass")}</span><span>pusula<span class="brand-sub">Bir ihtimal daha.</span></span></a>
    <div class="workspace-tag"><span class="tiny-dot"></span> ODTÜ · ÖĞRENCİ ALANI</div>
    <nav aria-label="Ana menü">${navItems
      .filter(([id]) => id !== "minor" || state.profile.minorEnabled)
      .map(
        ([id, i, label]) =>
          `<button class="nav-item ${ui.page === id ? "active" : ""}" data-page="${id}" ${ui.page === id ? 'aria-current="page"' : ""}>${icon(i)}<span>${label}</span>${ui.page === id ? '<span class="nav-dot"></span>' : ""}</button>`,
      )
      .join("")}</nav>
    <div class="sidebar-bottom"><div class="private-note">${icon("lock")}<span>Sadece senin tarayıcında.<br><strong>Hesap yok. Sunucu yok.</strong></span></div><button class="nav-item" data-action="data">${icon("data")}<span>Verilerim & paylaşım</span></button><button class="nav-item" data-action="settings">${icon("compass")}<span>Ayarlar & kaynaklar</span></button><div class="sidebar-footer">Sen planla. İhtimaller netleşsin.<span>ANKARA · ODTÜ</span></div></div>
  </aside>
  <div class="main-shell"><header class="topbar"><div class="breadcrumb">Makina Mühendisliği <span>/</span> <strong>${navItems.find(([id]) => id === ui.page)[2]}</strong></div><div class="topbar-actions"><span class="save-status ${saveError ? "warning-text" : ""}"><span class="tiny-dot"></span>${saveError ? "Kayıt başarısız" : "Bu tarayıcıda saklanır"}</span><button class="icon-button" data-action="undo" aria-label="Son değişikliği geri al" title="Son değişikliği geri al" ${!history.length ? "disabled" : ""}>${icon("undo")}</button><button class="icon-button" data-action="settings" aria-label="Plan ayarları" title="Plan ayarları">${icon("compass")}</button><button class="button small secondary" data-action="data">${icon("download")}<span>Yedekle / aktar</span></button></div></header>
  <main id="main">
    ...
    ${pages[ui.page]()}
    <footer class="main-footer"><span>${icon("compass")} Kendi yolunu, kendi varsayımlarınla çiz.</span><button data-action="sources">Hesaplama yöntemi & resmi kaynaklar ${icon("arrow")}</button></footer>
  </main></div>`;
  if (focusId) {
...
function pageHeading(eyebrow, title, description, actions = "") {
  return `<div class="page-heading"><div><div class="eyebrow">${eyebrow}</div><h1 id="page-heading" tabindex="-1">${title}</h1><p>${description}</p></div>${actions ? `<div class="heading-actions">${actions}</div>` : ""}</div>`;
}
...
function overviewPage() {
  ...
  return `${pageHeading("BÜYÜK RESMİ GÖR", "Önünde bir sürü ihtimal var.", "Notlarını yerleştir, ihtimalleri değiştir. Sonrasını birlikte hesaplayalım.", scenarioSelect())}
  ${
    !hasData()
      ? `<section class="welcome-card"><div class="welcome-copy"><span class="pill">BURADAN BAŞLA · 3 KÜÇÜK ADIM</span><h2>Geleceğin bir ortalamadan<br>çok daha fazlası.</h2><p>Önce aldığın notları gir. Sonra “ya şöyle olursa?” dediğin her şeyi güvenle dene.</p><div class="button-row"><button class="button primary" data-page="transcript">Notlarımı gireyim ${icon("arrow")}</button><button class="button text-button" data-action="example">Önce örneği keşfet</button></div></div><div class="welcome-art" aria-hidden="true"><svg viewBox="0 0 320 240"><defs><pattern id="dots" width="18" height="18" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r="1" fill="#b1bea2" opacity=".5"/></pattern></defs><rect width="320" height="240" fill="url(#dots)"/><circle cx="178" cy="126" r="83" fill="none" stroke="#bac5aa"/><circle cx="178" cy="126" r="56" fill="none" stroke="#bac5aa"/><path d="M10 215C75 214 63 148 113 159S169 204 197 96 250 60 316 19" fill="none" stroke="#426548" stroke-width="2.5"/><path d="M10 215C85 199 92 109 178 126S239 91 316 19" fill="none" stroke="#7c9669" stroke-dasharray="4 6"/><path d="m195 52-20 67-66 50 22-77z" fill="#204d3b"/><circle cx="153" cy="114" r="7" fill="#e7edce"/><circle cx="197" cy="96" r="6" fill="#dce99c" stroke="#204d3b" stroke-width="2"/></svg><span class="art-label">tek bir yol yok.</span></div></section>
  <div class="step-strip"><span><b>01</b> Gerçek notların</span>${icon("arrow")}<span><b>02</b> Senin varsayımların</span>${icon("arrow")}<span><b>03</b> Daha net kararlar</span></div>`
      : ""
  }
  ...
  <div class="dashboard-columns"><section class="panel trajectory-panel"><div class="panel-heading"><div class="eyebrow">BUNDAN SONRASI</div><span class="pill neutral">${p.plannedCount} planlanan ders</span></div><h2>Rotan nasıl değişiyor?</h2><p class="muted">Seçtiğin dersler eklendikçe anadal ortalaman.</p>${trajectoryChart()}<div class="chart-legend"><span><i class="legend-dot"></i>Mevcut + senaryo</span><span><i class="legend-line"></i>Hedef: ${num(state.profile.target)}</span></div>${p.remainingUnplanned ? `<div class="inline-note">${icon("info")} ${p.remainingUnplanned} anadal gerekliliği henüz planlanmadı. Bu sonuç tam mezuniyet tahmini değil.</div>` : `<div class="inline-note">${icon("info")} Ders planı hesabıdır; mezuniyet onayı yerine geçmez.</div>`}</section>
  <section class="panel goal-panel"><div class="eyebrow">AKLINDA BİR HEDEF Mİ VAR?</div><h2>“${num(state.profile.target)} ile<br>bitirebilir miyim?”</h2><label class="target-label" for="target-gpa">Hedef genel not ortalaması <output>${num(state.profile.target)}</output></label><input id="target-gpa" type="range" min="2" max="4" step=".05" value="${state.profile.target}"><div class="range-labels"><span>2,00</span><span>3,00</span><span>4,00</span></div><div class="goal-result">${target.remainingCredits > 0 ? `<span>Kalan ${num(target.remainingCredits, 0)} kredide gereken ortalama</span><strong>${num(target.requiredAverage)}</strong><p>${target.requiredAverage > 4 ? "Bu planla hedef 4,00 üstü gerektiriyor. Bir ders tekrarı veya başka bir hedef deneyebilirsin." : target.requiredAverage <= 0 ? "Mevcut varsayımların hedefi karşılıyor. Kalan dersleri de geçmen gerekir." : "Seçtiğin tahminlerin beklenen değeriyle. Tekrarlar da hesaba katılır."}</p>` : `<span>${p.plannedCount ? "Bu planın hedefe göre durumu" : "Planlamak için notlarını gir"}</span><strong>${p.gpa == null ? "—" : p.gpa >= state.profile.target ? "Hedefte" : "Hedef altında"}</strong><p>Tahminleri veya ders tekrarlarını değiştirebilirsin.</p>`}</div><button class="button primary full-width" data-page="scenarios">Bir senaryo deneyelim ${icon("arrow")}</button></section></div>
  <div class="section-title"><div><span class="eyebrow">KÜÇÜK SORULAR, DAHA NET CEVAPLAR</span><h2>Bugün neyi merak ediyorsun?</h2></div></div><div class="question-grid">
    ${questionCard("repeat", "“PHYS 105’i tekrar alsam?”", "Şimdi kaç puan, mezuniyette kaç puan? Emeğinin sayısal karşılığını gör.", "retakes", "sage")}
    ${questionCard("chart", "“Finalden kaç almalıyım?”", "Vize, ödev ve final ağırlıklarını gir. İhtiyacın olan notu hemen bul.", "exams", "peach")}
    ${questionCard("branch", "“Kalan dersler BB gelse?”", "Farklı planlar oluştur, yan yana karşılaştır. Gerçek notlarına dokunmadan.", "scenarios", "cream")}
  </div>`;
}
function questionCard(i, title, description, page, color) {
  return `<button class="question-card ${color}" data-page="${page}"><span class="question-icon">${icon(i)}</span><h3>${title}</h3><p>${description}</p><span class="question-arrow">Deneyelim ${icon("arrow")}</span></button>`;
}
...
function transcriptPage() {
  ...
  return `${pageHeading("ÖNCE GERÇEKLER", "Her şey senin notlarınla başlar.", "Transkriptindeki son geçerli notları gir. Tahminlerini Senaryolarım’da dene.", `<button class="button secondary" data-action="add-course">${icon("plus")} Ders ekle</button>`)}
...
function scenariosPage() {
  ...
  return `${pageHeading(`${Math.ceil(state.profile.graduationSemester / 2)} YILLIK PLANIN · HER DÖNEM YENİ BİR İHTİMAL`, "Bir plan. Bir başka ihtimal.", "Gerçek notların sabit kalır. Buradaki her not, değiştirebileceğin bir varsayım.", `<button class="button secondary" data-action="random-fill">${icon("spark")} Rastgele doldur</button><button class="button primary" data-action="new-scenario">${icon("plus")} Yeni senaryo</button>`)}
...
function retakesPage() {
  ...
  const c = eligible.find((c) => c.id === ui.retakeId),
    impact = c
      ...
      : null;
  return `${pageHeading("EMEĞİNE DEĞER Mİ?", "“Bir daha alsam ne değişir?”", "Bir dersteki küçük farkı, bütün planın içinde gör.")}
  ${
    !c
...
function examsPage() {
  ...
  return `${pageHeading("BİR SONRAKİ SINAVA KADAR", "“Finalden kaç almam lazım?”", "Dersin gerçek ağırlıklarını gir. Boş notlar, henüz alınmamış notlardır.")}
  ...
  <section class="panel letter-link"><div>${icon("branch")}<div><h3>Bunu genel ortalamama bağlamak istiyorum.</h3><p>Sayısal puanın harf karşılığını dersin değerlendirme kuralına göre kendin seç.</p></div></div><div class="button-row">${gradeSelect(c.id, e.grade || "", "plan")}<button class="button secondary" data-page="scenarios">Senaryomu gör ${icon("arrow")}</button></div></section><div class="inline-note">${icon("info")} ODTÜ genelinde “80 = BA” gibi sabit bir eşik varsaymıyoruz. Bağıl değerlendirme ve harf sınırları derse göre değişebilir.</div>`;
}
function examResult(assessments, target) {
  const result = weightedExams(assessments, target);
  if (!result.valid)
    return `<span class="eyebrow">BİR KÜÇÜK DÜZELTME</span><h2>Önce ağırlıkları<br>tamamlayalım.</h2><div class="inline-note error-note">${esc(result.error)}</div><p>Ağırlıklar toplamı %100, her not 0–100 arasında olmalı.</p>`;
  const secured =
...
function minorPage() {
  ...
  return `${pageHeading("ANADALIN YANINDA BİR YOL DAHA", "Mekatronik, ayrı bir hikâye.", "Yandal notlarını ve kalan gerekliliklerini anadaldan ayrı takip et.")}
...
function openDialog(title, body, wide = false) {
  ...
  dialog.innerHTML = `<div class="dialog-heading"><div><span class="eyebrow">PUSULA · SENİN ALANIN</span><h2 id="dialog-title">${title}</h2></div><button class="icon-button" data-action="close-dialog" aria-label="Pencereyi kapat">${icon("close")}</button></div><div class="dialog-content">${body}</div>`;
  dialog.showModal();
}
...
function downloadJSON() {
  ...
  link.href = url;
  link.download = `pusula-${new Date().toISOString().slice(0, 10)}.json`;
  link.click();
